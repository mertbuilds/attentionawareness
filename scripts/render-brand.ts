...
for (const icon of icons) {
  ...
  process.stdout.write(`render-brand: ${icon.file} (${icon.size}x${icon.size})\n`);
}
...
// The same outlines the favicon is cut from, so the mark on the page and the
// mark in the browser tab can never drift apart.
writeFileSync(
  path.join(root, 'apps/web/src/components/brand-mark-paths.ts'),
  [
...
process.stdout.write('render-brand: brand-mark-paths.ts\n');
