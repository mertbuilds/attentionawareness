...
UPDATE_URL = "https://attentionawareness.com/cli/supervise"
...
def plan_patch(original: bytes, recorded_size: int, supervised: bool = True) -> PatchPlan:
    """Work out the new file bytes without writing anything.

    `supervised` is the state the backup should say afterwards: true to
    supervise, false to take supervision off again.
    """
    ...
    wanted = "true" if supervised else "false"
    if content.get("IsSupervised") is not supervised:
        plan.changes.append(f"IsSupervised: {plist_value(content.get('IsSupervised'))} -> {wanted}")
        content["IsSupervised"] = supervised
    if supervised and content.get("CloudConfigurationUIComplete") is False:
...
def verify_patch(backup: Backup, supervised: bool = True) -> int:
    ...
    if content.get("IsSupervised") is not supervised:
        wanted = "true" if supervised else "false"
        raise Refusal(f"Verification failed. IsSupervised is not {wanted}. Run the unpatch command.")
...
def print_checklist(backup: Backup, supervised: bool = True) -> None:
    print("")
    ...
    if supervised:
        ...
        print("     The supervision banner at the top must be gone, and with it")
        print("     every profile that supervision held in place.")
...
def cmd_patch(args) -> int:
    return patch_to(args, supervised=True)


def cmd_unsupervise(args) -> int:
    return patch_to(args, supervised=False)


def patch_to(args, supervised: bool) -> int:
    """Set the backup's supervision flag one way or the other, then verify it."""
    ...
    if backup.is_supervised is supervised:
        state = "supervised" if supervised else "not supervised"
        print(f"This backup already says that the iPhone is {state}. Nothing to change.")
        return EXIT_OK

    plan = plan_patch(read_content(backup), backup.recorded_size, supervised)
    refuse_slow_manifest_rewrite(backup, plan)
    ...
    size = verify_patch(backup, supervised)
    wanted = "true" if supervised else "false"
    print(f"Patched. IsSupervised is {wanted} and the file is {size} bytes, as Manifest.db records.")
    print_checklist(backup, supervised)
    return EXIT_OK
...
def build_parser() -> argparse.ArgumentParser:
    ...
    unsupervise = subparsers.add_parser(
        "unsupervise",
        help="Set IsSupervised to false in a backup, to take supervision off again.",
    )
    unsupervise.add_argument("--udid", help="Pick the backup with this UDID.")
    unsupervise.add_argument("--yes", action="store_true", help="Do not ask for confirmation.")
    unsupervise.add_argument("--password", help=PASSWORD_HELP)
    unsupervise.set_defaults(handler=cmd_unsupervise, json=False)
