...
export const Route = createFileRoute('/guides/supervise-iphone-without-erasing')({
  ...
  head: () => ({
    meta: [
      { title: `${m.guide_sup_head_title()} · ${SITE_NAME}` },
      { content: m.guide_sup_description(), name: 'description' },
      { content: m.guide_sup_title(), property: 'og:title' },
      { content: m.guide_sup_description(), property: 'og:description' },
...
const PAGE_URL = 'https://attentionawareness.com/guides/supervise-iphone-without-erasing';
...
const styles = create({
  ...
  // The one boxed thing on the page: the shorter way to the same place,
  // offered once at the top and then left alone.
  promo: {
    borderColor: colors.border,
    borderRadius: radius.base,
    borderStyle: 'solid',
    ...
    <main {...props(styles.page)}>
      ...
      <article {...props(styles.article)}>
        <header {...props(styles.hero)}>
          ...
          <h1 {...props(styles.heroTitle)}>{m.guide_sup_title()}</h1>
          <p {...props(styles.byline)}>
            {m.guide_sup_byline({ author: AUTHOR_NAME, date: UPDATED_LABEL })}
          </p>
          <p {...props(styles.lead)}>{m.guide_sup_intro()}</p>
        </header>

        <div {...props(styles.content)}>
          <section {...props(styles.section, styles.promo)}>
              ...
              {checklist.map((item) => (
                <li key={item.id} {...props(styles.checkItem)}>
                  <Label style={styles.checkLabel}>
                    ...
                    />
                    <span {...props(checked.includes(item.id) && styles.checkDone)}>
                      {item.text}
                    </span>
                  </Label>
                </li>
