 ...
 * write-a-patch shape the real one has. The two calls the popup's footer makes
 * come back as spies, because what matters about them is that they were made.
 */
export function mockChrome(initial: Record<string, unknown> = {}) {
  ...
  return { createTab, listeners, openOptionsPage, store };
}
